...
from build_windows_release import audit, validate_app
p=argparse.ArgumentParser(); p.add_argument("package"); a=p.parse_args(); root=Path(a.package).resolve()
load_release(root/"release-manifest.json",root); validate_app(root/"dist/app"); audit(root); print(f"Verified {root}")
