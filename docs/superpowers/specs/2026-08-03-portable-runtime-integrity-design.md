...
- `scripts/launcher/tests/test_lifecycle_contract.py`
  - contract tests for receipt integrity and recovery ordering.
...
2. Add failing launcher contract tests for receipt content and recovery-before-download ordering.
3. Add failing Windows E2E steps for a corrupted installed file and offline `python.previous` recovery.
4. Implement strict Python manifest validation.
5. Implement receipt file hashing and installed-runtime verification in PowerShell.
6. Implement startup recovery and recoverable publication.
7. Update scenario documentation with the actual CI status.
8. Run the complete repository and Windows release gates.
9. Perform an independent read-only code review before opening the correcting PR.
