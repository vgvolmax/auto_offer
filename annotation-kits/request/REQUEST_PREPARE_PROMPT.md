...
Для обычного текстового документа `raw_text` — исходная товарная строка без
semantic rewrite. Для таблицы товарная позиция — **вся логическая строка
таблицы**, а не только колонка «Наименование». Сохрани в одном `raw_text` все
непустые product-defining cells этой строки. К ним как минимум относятся
«Наименование», «Тип», «Марка», «Тип/марка», «Модель», «Артикул», «Код
изделия», «Производитель», «Изготовитель», «Бренд», «Стандарт», «ГОСТ», «ТУ»,
«Документ», «Исполнение», «Материал», «Размер», «Характеристики», «Описание»,
«Примечание» и «Дополнительные требования». Это не исчерпывающий whitelist:
нельзя потерять соседнюю ячейку с информацией о товаре только потому, что её
заголовок не перечислен.

Используй lossless serialization: сохраняй значения ячеек дословно, доступные
исходные названия колонок и их порядок, технически соединяя пары стабильным
разделителем ` | `. Например:

`Наименование: Фитинг полипропиленовый - тройник переходной 25x20x25 мм | Тип/марка: VTp.735.0.025020025 | Производитель: VALTEC | Примечание: или эквивалент`

Запрещены semantic rewrite, сокращение данных, самостоятельное исправление
обозначений и нормализация артикулов или моделей. Если название находится в
одной колонке, article/model — в другой, manufacturer — в третьей, а
примечание — в четвёртой, всё это относится к одной `request-source` line.
Запрещено создавать `raw_text` только из значения «Наименование», когда в той
же source row есть другие product-defining values.

Обязательно сохраняй в `raw_text` substitution information из «Примечания»
или любой соседней ячейки: «или эквивалент», «или аналог», «аналог допускается»,
«замена допускается», «замена не допускается», «строго указанная марка» и
аналогичные формулировки. На STEP 1 не интерпретируй их, а только сохраняй
source.

Не превращай `raw_text` в dump всей страницы: не включай простой порядковый
номер строки, пустые cells, headers других товарных строк, итоги, подписи,
реквизиты документа или иные служебные поля, не относящиеся к товару.
Количество остаётся отдельно в `quantity_raw`. Если количество и единица
находятся в разных ячейках, совместно сохрани их без semantic conversion:
`Количество: 20`, `Ед.: шт` → `quantity_raw = "20 шт"`.
