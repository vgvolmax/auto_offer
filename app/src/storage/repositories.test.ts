...
import { matchRunsRepository, createMatchRunsRepository } from "./match-runs-repository";
...
describe("IndexedDB repositories — A2-A5, H1, G5", () => {
  ...
  it(
    "upgrades a v2 database and lazily preserves one selection state per run",
    async () => {
      const c = createCatalogRecord(catalog as any);
      const s = createDraftSession(request as any, [c], "v2");
      const run: any = { id: "v2-run", sessionId: s.sessionId, sessionRevision: 0, createdAt: "2026-01-01T00:00:00.000Z", result: expected };
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        open.onupgradeneeded = () => {
          ...
          db.createObjectStore("catalogs", { keyPath: "recordId" });
          db.createObjectStore("sessions", { keyPath: "sessionId" });
          db.createObjectStore("matchRuns", { keyPath: "id" }).createIndex("by-session", "sessionId");
          db.createObjectStore("settings", { keyPath: "key" });
          open.transaction!.objectStore("catalogs").put(c);
      ...
      expect(db.transaction("selectionStates").store.indexNames.contains("by-session")).toBe(true);
      expect(await catalogsRepository.get(c.recordId)).toEqual(c);
      expect(await sessionsRepository.get(s.sessionId)).toMatchObject({ latestMatchRunId: run.id });
      expect(await matchRunsRepository.get(run.id)).toEqual(run);
      const state = await selectionStatesRepository.getOrCreateForRun(run);
      expect(state).toMatchObject({ matchRunId: run.id, sessionId: run.sessionId, inputFingerprint: expected.input_fingerprint, revision: 0, decisions: {} });
      const saved = await selectionStatesRepository.saveDecision({ sessionId: s.sessionId, matchRunId: run.id, lineId: "line", expectedRevision: 0, decision: { kind: "selected_offer", offerRef: (expected as any).lines[0].candidates[0].offer_ref, confirmedAt: "2026-01-01" } });
  ...
  it(
    "atomically replaces the latest run and starts with an empty selection state",
    async () => {
      const c = createCatalogRecord(catalog as any);
      const s = createDraftSession(request as any, [c], "atomic");
      await catalogsRepository.save(c); await sessionsRepository.save(s);
      const a = await matchRunsRepository.saveLatest({ sessionId: s.sessionId, expectedSessionRevision: 0, result: expected as any });
      await selectionStatesRepository.saveDecision({ sessionId: s.sessionId, matchRunId: a.id, lineId: "line", expectedRevision: 0, decision: { kind: "selected_offer", offerRef: (expected as any).lines[0].candidates[0].offer_ref, confirmedAt: "2026-01-01" } });
      const b = await matchRunsRepository.saveLatest({ sessionId: s.sessionId, expectedSessionRevision: 0, result: { ...expected, input_fingerprint: "b" } as any });
      expect(await matchRunsRepository.get(a.id)).toBeUndefined();
      expect(await selectionStatesRepository.get(a.id)).toBeUndefined();
      expect(await selectionStatesRepository.get(b.id)).toMatchObject({ revision: 0, decisions: {} });
      expect(await sessionsRepository.get(s.sessionId)).toMatchObject({ latestMatchRunId: b.id });
      const bState = await selectionStatesRepository.saveDecision({ sessionId: s.sessionId, matchRunId: b.id, lineId: "line", expectedRevision: 0, decision: { kind: "selected_offer", offerRef: (expected as any).lines[0].candidates[0].offer_ref, confirmedAt: "kept" } });
      const failing = createMatchRunsRepository({ createId: () => "run-c", createSelectionStateForRun: () => { throw new Error("transaction failure"); } });
      await expect(failing.saveLatest({ sessionId: s.sessionId, expectedSessionRevision: 0, result: { ...expected, input_fingerprint: "c" } as any })).rejects.toThrow("transaction failure");
      expect(await sessionsRepository.get(s.sessionId)).toMatchObject({ latestMatchRunId: b.id });
