...
const remaining=(annotation:NonNullable<NonNullable<CatalogRecord['bundle']['items'][number]['catalog_item']>['annotation']>)=>
  new Set([...(annotation.unknown_fields??[]),...(annotation.issues??[]).map(x=>x.json_pointer??x.code??'issue'),...(annotation.ambiguities??[]).filter(x=>x.blocking!==false).map(x=>x.json_pointer??x.code??'ambiguity')]).size;
...
export function applyCatalogReviewEdits(input:{record:CatalogRecord;sourceItemId:string;edits:CatalogManualEdit[];now:string}):CatalogReviewEditResult{
  ...
  const registration=(createBrowserValidationContext().registry.classes as Record<string,{allowed_annotation_paths?:string[]}>)[item.class_id];
  const allowed=(registration as {allowed_annotation_paths?:string[]}|undefined)?.allowed_annotation_paths??[];
  for(const edit of input.edits){
    if(!allowed.some(pattern=>matches(pattern,edit.jsonPointer))||!setAt(item,edit.jsonPointer,edit.value))return{ok:false,code:'EDIT_PATH_NOT_ALLOWED',message:`Поле ${edit.jsonPointer} нельзя редактировать`};
    const annotation=item.annotation!;
    annotation.unknown_fields=(annotation.unknown_fields??[]).filter(pointer=>pointer!==edit.jsonPointer);
    annotation.ambiguities=(annotation.ambiguities??[]).filter(value=>value.json_pointer!==edit.jsonPointer);
    annotation.issues=(annotation.issues??[]).filter(value=>value.json_pointer!==edit.jsonPointer);
  ...
  const reviewCount=remaining(item.annotation!);item.annotation!.status=reviewCount?'needs_review':'validated';
