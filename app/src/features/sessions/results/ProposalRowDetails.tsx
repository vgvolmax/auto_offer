...
export function ProposalRowDetails(p: {
  ...
  return <div className="proposal-details">
    ...
    {line.candidates.map((candidate, index) => <CandidateCard key={candidate.key} candidate={candidate} lineId={line.lineId} disabled={p.disabled} baselineReady={line.effectiveOutcome?.kind === "selected_offer" && line.effectiveOutcome.source === "ai"} operatorOverride={line.hasDecision && line.runKind === "semantic"} onSelect={() => p.onSelect(index)} onClear={p.onClear} />)}
